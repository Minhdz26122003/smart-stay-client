...
*   **State Managers (BLoC/Cubit):** Manage the UI state. Consume domain models from the Data/Logic layers and transform them into presentation-friendly formats. Expose state to the Views and handle user interaction events.
...
*   **If the application is a standard CRUD app:** Omit this layer. Allow BLoC/Cubit to interact directly with Repositories.
...
- [ ] **Step 2: Implement Services.** Create stateless Service classes to handle raw data fetching.
...
- [ ] **Step 4: Implement BLoC/Cubit.** Create states and logic managing classes that consume the Repositories. Expose immutable state and handle events.
- [ ] **Step 5: Implement Views.** Create Flutter Widgets that bind to the State using `BlocBuilder` or `BlocConsumer` and trigger Cubit/Bloc methods on user interaction.
- [ ] **Step 6: Run Validator.** Execute unit tests for Services, Repositories, and blocs. Execute widget tests for Views.
...
// 2. Domain Model (Immutable - use Freezed in real app)
class User {
  final String id;
  final String name;
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<UserCubit, UserState>(
      builder: (context, state) {
        ...
        if (state.user == null) return const Text('No user data.');
